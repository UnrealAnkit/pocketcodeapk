...
fun QrScannerScreen(onPaired: (PairingQR) -> Unit, onManual: () -> Unit) {
    ...
    // One-shot guard so a valid pairing code doesn't fire onPaired on every camera frame.
    ...
    Column(Modifier.fillMaxSize().padding(16.dp)) {
        ...
        if (!hasPermission) {
            ...
            AndroidView(factory = { c ->
                ...
                val mainExecutor = ContextCompat.getMainExecutor(c)
                    ...
                    val analyzer = ImageAnalysis.Builder().build().also {
                        it.setAnalyzer(analysisExecutor, QrAnalyzer { raw ->
                            ...
                            mainExecutor.execute {
                                if (pairing == null) {
                                    // Do not consume the scanner for an unrelated QR code: the next frame
                                    // may contain the real PocketCode pairing code.
                                    scanMessage = "This isn't a PocketCode pairing QR. Try the code from your computer."
                                } else if (consumed.compareAndSet(false, true)) {
                                    onPaired(pairing)
                                }
                            }
                        })
